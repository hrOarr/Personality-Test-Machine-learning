...
from .serializers import ArticleCreateSerializer, ArticleListSerializer, ArticleUpdateSerializer
...
def article_create_view(request):
    """View To Create New Article For The Logged In Users"""
...
class ArticleListView(generics.ListAPIView):
    """List View For Listing All The Articles Of A Particular User"""
...
def article_update_view(request):
    """View To Update A Article For Logged In Users"""
    ...
    if request.method == 'POST':
        ...
        instance = Article.objects.get(slug=updated_data.get('slug'))
        admin_user = User.objects.get(pk=2)  # PK Of Admin User Is 1

        if(instance.author == logged_in_user or logged_in_user == admin_user):
            updated_data.pop('slug')
            serializer = ArticleUpdateSerializer(instance, data=updated_data)
...
def article_delete_view(request):
    ...
    if request.method == 'DELETE':
        ...
        instance = Article.objects.get(slug=request.data.get('slug'))
