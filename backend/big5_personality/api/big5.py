...
class Big5:
    def __init__(self):
        self.df = pd.read_csv(r'C:\wamp64\www\ReactApp\techleader\backend\big5_personality\api\data\Big5\big5_data.csv', sep='\t')
        self.prepare_df()
        self.questions_key = {
    ...
    def handle_personality_test(self, answers):
        ...
        for trait_key, val in answer_dict.items():
            if 'O' in trait_key:
                score_dict['O_score'] += val
            if 'C' in trait_key:
                score_dict['C_score'] += val
            if 'E' in trait_key:
                score_dict['E_score'] += val
            if 'A' in trait_key:
                score_dict['A_score'] += val
            if 'N' in trait_key:
                score_dict['N_score'] += val
        ...
        res_dict = {}
        res_dict['percentiles'] = perc_dict
        res_dict['scores'] = score_dict
    ...
    def prepare_df(self):
        ...
        self.df['O_score'] = self.cal_score(self.df[O_columns])
        self.df['C_score'] = self.cal_score(self.df[C_columns])
        self.df['E_score'] = self.cal_score(self.df[E_columns])
        self.df['A_score'] = self.cal_score(self.df[A_columns])
        self.df['N_score'] = self.cal_score(self.df[N_columns])
